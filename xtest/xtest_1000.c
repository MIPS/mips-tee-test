...
static void xtest_tee_test_1009_subcase_2(ADBG_Case_t *c, const char *subcase,
                                          uint32_t timeout)
{
	TEEC_Session session = { 0 };
	uint32_t ret_orig;
	...
	Do_ADBG_BeginSubCase(c, "%s", subcase);
	{
		...
		op.params[0].value.a = timeout;
		op.params[0].value.b = true; // unmask cancel
		op.params[3].value.a = TA_SISS_CMD_WAIT;
		op.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT,
						 TEEC_NONE,
						 TEEC_NONE,
						 TEEC_VALUE_INPUT);
		...
		res = xtest_teec_open_session(&session, &siss_ta_uuid, &op,
				&ret_orig);
...
static void xtest_tee_test_1009(ADBG_Case_t *c)
{
	...
	xtest_tee_test_1009_subcase_2(c,
			"TEE Wait 2s cancel open session", 2000);
#endif
...
#ifdef WITH_LARGE_CONCURRENT_TA
